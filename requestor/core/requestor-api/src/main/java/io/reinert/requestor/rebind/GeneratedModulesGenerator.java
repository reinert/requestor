...
public class GeneratedModulesGenerator extends Generator {
    ...
    private final JsonAutoBeanGenerator autoBeanGenerator = new JsonAutoBeanGenerator();
    ...
    public String generate(TreeLogger logger, GeneratorContext ctx, String typeName) throws UnableToCompleteException {
        ...
        if (sourceWriter != null) {
            ...
            boolean generateAutoBeans = false;
            try {
                ConfigurationProperty prop = ctx.getPropertyOracle().getConfigurationProperty("generateAutoBeans");
                if (!prop.getValues().isEmpty() && prop.getValues().get(0).equalsIgnoreCase("true")) {
                    generateAutoBeans = true;
                }
            } catch (BadPropertyValueException e) {
                logger.log(TreeLogger.ERROR, "Could not get property generateAutoBeans", null);
            }
