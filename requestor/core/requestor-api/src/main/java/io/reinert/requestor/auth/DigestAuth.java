...
public class DigestAuth implements Authentication {
    ...
    public void authenticate(final RequestOrder request) {
        ...
        final Promise<Headers> promise = requestor.dispatch(new SerializedRequestImpl(request.getMethod(),
                request.getUrl()), Headers.class);

        promise.then(new Callback<Headers, Void>() {
            @Override
            public Void call(Headers result) {
                ...
                int realmEndIdx = authHeader.indexOf("\",", realmStartIdx) + 2;
                ...
                int nonceEndIdx = authHeader.indexOf("\",", nonceStartIdx) + 2;
                ...
                // Read QoP
                int qopStartIdx = authHeader.indexOf("qop=\"") + 7;
                final String qop = qopStartIdx != 6 ?
                        authHeader.substring(qopStartIdx, authHeader.indexOf("\",", qopStartIdx) + 2) : null;

                if (qop != null) {
                    // Append QoP
                    authBuilder.append("\", qop=\"").append(qop);
                }
                ...
                final String cNonce = nc + nonce + Duration.currentTimeMillis() + getRandom();
                ...
                if (qop != null) {
                    if (qop.contains("auth-int") && !qop.contains("auth")) {
                        // "Auth-Int" method
                        final String stringPayload = request.getPayload().isString();
                ...
                int opaqueStartIdx = authHeader.indexOf("opaque=\"") + 7;
                int opaqueEndIdx = authHeader.indexOf("\",", opaqueStartIdx) + 2;
