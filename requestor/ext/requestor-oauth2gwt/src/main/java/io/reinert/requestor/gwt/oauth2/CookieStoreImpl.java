...
class CookieStoreImpl extends TokenStoreImpl {
    ...
    public native void set(String key, String value) /*-{
        $doc.cookie = @io.reinert.requestor.gwt.oauth2.CookieStoreImpl::COOKIE_PREFIX +
        encodeURIComponent(name) + '=' + encodeURIComponent(value);
    }-*/;

    @Override
    public native String get(String key) /*-{
        var m = @io.reinert.requestor.gwt.oauth2.CookieStoreImpl::ensureCookies();
        return m[@io.reinert.requestor.gwt.oauth2.CookieStoreImpl::COOKIE_PREFIX + key];
    }-*/;

    @Override
    public native void clear() /*-{
        var allCookies = @io.reinert.requestor.gwt.oauth2.CookieStoreImpl::ensureCookies()();
        var prefix = @io.reinert.requestor.gwt.oauth2.CookieStoreImpl::COOKIE_PREFIX;

        for (var key in allCookies) {
            if (key.indexOf(prefix) == 0) {
                this.@io.reinert.requestor.gwt.oauth2.CookieStoreImpl::set(*)(
                    key.substring(prefix.length), '');
            }
        }
        @io.reinert.requestor.gwt.oauth2.CookieStoreImpl::cachedCookies = null;
    }-*/;
    ...
    private static native void loadCookies() /*-{
        @io.reinert.requestor.gwt.oauth2.CookieStoreImpl::cachedCookies = {};
        ...
        if (docCookie && docCookie != '') {
            ...
            for (var i = 0; i < crumbs.length; ++i) {
                ...
                @io.reinert.requestor.gwt.oauth2.CookieStoreImpl::cachedCookies[name] = value;
            }
        }
    }-*/;
